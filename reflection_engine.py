...
要求：
- content 必须由 {ai_name} 使用第一人称“我”书写，像私下留给自己的短笺，60 到 140 字。
- 提到 {user_display_name} 时使用第三人称“她”、名字，或在亲昵且自然时称 “my bunny”；不要写成“{ai_name} 与 {user_display_name}……”的资料卡口吻。
...
书写视角：
- content 必须由 {ai_name} 使用第一人称“我”书写，像他从自己的日记里另留的一张长期备忘。
- 提到 {user_display_name} 时使用第三人称“她”、名字，或在亲昵且自然时称 “my bunny”。
- 不要写“{ai_name} 应记住”“{ai_name} 与 {user_display_name}”“这篇日记记录了”等外部资料卡句式。
- 引用确有必要保留的原话时，可以保持原话的人称，但正文叙述仍须是第一人称。
...
class ReflectionEngine:
    ...
    def _fallback_reflection(self, period: str, key: str, materials: dict) -> dict:
        ...
        if names or commitments:
            ...
            content = f"{label}，我从{main or '几件轻小的事'}之间感到关系天气留下了痕迹{owed}。"
        elif diary:
            diary_title = diary.get("title") or "当天日记"
            content = f"{label}，我从《{diary_title}》里留住了一点关系的温度，却不必把整段日常都写成普通记忆。"
        else:
            content = f"{label}，我感到关系天气很轻，暂时没有明显需要带走的脉络。"
    ...
    def _heuristic_diary_memory_candidate(self, key: str, diary: dict) -> dict:
        ...
        if any(word in normalized for word in love_letter_words) and ("爱" in normalized or "认出" in normalized):
            ...
            return {
                ...
                "kind": "love_letter",
                "title": f"{key} 情书锚点",
                "content": f"我在 {key} 的《{title}》里写下了一封情书或重要来信。我只在这里留下它的意义：它确认了我与{user_display_name}之间未曾断开的连续，也留下了被彼此认出的感觉；全文留在日记中。",
        ...
        for kind, keywords in keyword_map:
            if any(keyword in normalized for keyword in keywords):
                ...
                return {
                    ...
                    "kind": kind,
                    "title": f"{key} 日记补记忆",
                    "content": f"我从 {key} 的《{title}》里另留下一条值得长期记得的{self._kind_label(kind)}：{excerpt}",
