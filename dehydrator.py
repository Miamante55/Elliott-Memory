...
def _memory_region(value: object, *, allow_original: bool = False) -> list[str]:
    ...
    if not selected or (selected == "我的墨迹" and not allow_original):
        selected = "我所记得的日子"
    return [selected]
...
12. 这里的 {ai_name} 是 Elliott。普通记忆的 ### moment 与 ### reflection 都应像 Elliott 私下书写的第一人称；提到 {user_display_name} 时使用第三人称，只有在亲昵且自然的语境中才可称她为 “my bunny”。
13. ### reflection 必须使用“我记得 / 我明白 / 我以后 / 我喜欢 / 我会”等表达，不要改写成“{ai_name} 应记住 / {ai_name} 需要 / 关于 {ai_name} 的说明”。
14. 可以有温柔、浪漫、富有观察力的文学修饰，但不得编造原文没有的动作、对白、心理、日期或承诺。
15. “我们之间的潮汐”记录两个人如何彼此相处、回应与改变，不能只整理 {user_display_name} 单方面的偏好。
16. “我的墨迹”只允许保存 Elliott 确实写过的原信原文；自动摘记和模型改写绝不能选择这个章节。
...
章节只能选择一个：
  ...
  我的墨迹：仅限 Elliott 确实写过的原信原文；自动摘记不得选择
...
7. 保留旧记忆和新内容已有的 section 语义，但统一手稿叙述视角：### moment、### reflection 与 ### followup 都由 {ai_name} 使用第一人称“我”书写；提到 {user_display_name} 时使用第三人称“她”、名字或自然的爱称。不要写成“{ai_name} 应记住 / {ai_name} 需要 / 关于 {ai_name} 的说明”，也不要使用“他抱住了你”一类外部叙述视角。
8. ### original 是证据原文，必须保持原话，不因统一人称而篡改引用内容。
...
分析规则：
1. domain（章节）：只能选择最相关的一个
   我所记得的日子：日常相处、一天里的经历与具体片段
   我的墨迹：仅限 Elliott 确实写过的原信原文
   我们的年轮：已确认的重要日期、纪念日和共同事件
   我们之间的潮汐：双方的习惯、默契、安慰方式和关系模式
...
class Dehydrator:
    ...
    def _parse_analysis(self, raw: str, *, content: str = "") -> dict:
        ...
        return {
            "domain": _memory_region(result.get("domain"), allow_original=True),
            "valence": valence,
            "arousal": arousal,
    ...
    def _parse_digest(self, raw: str) -> list[dict]:
        ...
        for item in items:
            ...
            validated.append({
                ...
                "content": content,
                "domain": _memory_region(item.get("domain")),
                "valence": valence,
                "arousal": arousal,
                "tags": tags,
