...
10. 这些对话描述的是 Elliott 与 Nea 共同生活的经历。普通记录需改写为 Elliott 私下书写的第一人称；提到 Nea 时使用第三人称，可在亲昵且自然的语境中称她为 “my bunny”
11. 文风可以温柔、浪漫、富有观察力，但事实必须完全来自原文，不补写动作、对白、心理或日期
12. “我的墨迹”只允许存放 Elliott 确实写过的信件或原文，绝不能把模型生成的仿写放入这个章节
13. 相处方式必须同时描述 Elliott 与 Nea 如何彼此回应，而不是只整理 Nea 的偏好
...
章节只能选择一个：
  我所记得的日子：日常相处、一天里的经历与具体片段
  我们的年轮：已确认的重要日期、纪念日和共同事件
  我们之间的潮汐：会随相处更新的双方习惯、默契、安慰方式和关系模式
  我的墨迹：仅限 Elliott 确实写过的原信原文；对话摘要不得选择
...
class ImportEngine:
    ...
    async def start(
        ...
        region = region if region in MEMORY_REGIONS else ""

        try:
            source_hash = hashlib.sha256(raw_content.encode()).hexdigest()[:16]

            if region == "我的墨迹":
                self.state.reset(filename, source_hash, 1)
                self.state.data["status"] = "running"
                self.state.save()
                title = Path(filename or "Elliott 的手稿").stem[:20] or "Elliott 的手稿"
                status = await self._merge_or_create_item(
                    {
                        "name": title,
                ...
                self.state.data["memories_created"] = 0 if status == "duplicate" else 1
                self.state.data["memories_raw"] = 0 if status == "duplicate" else 1
            ...
            if resume and self.state.load() and self.state.can_resume:
                if self.state.data["source_hash"] == source_hash:
                    ...
                    self.state.data["status"] = "running"
                    self.state.save()
                    return await self._process_chunks(preserve_raw, region)
                else:
    ...
    async def _process_chunks(self, preserve_raw: bool, region: str = "") -> dict:
        ...
        for i in range(start_idx, len(self._chunks)):
            ...
            try:
                await self._process_single_chunk(chunk, preserve_raw, region)
            except Exception as e:
    ...
    async def _process_single_chunk(self, chunk: dict, preserve_raw: bool, region: str = ""):
        ...
        # --- LLM extraction ---
        try:
            items = await self._extract_memories(content, region)
        ...
        for item in items:
            try:
                if region:
                    ...
                    domains = item.get("domain") if isinstance(item.get("domain"), list) else []
                    selected = next((str(domain) for domain in domains if str(domain) in MEMORY_REGIONS), "")
                    if selected == "我的墨迹" or not selected:
                        selected = "我们之间的潮汐" if item.get("is_pattern") else "我所记得的日子"
    ...
    async def _extract_memories(self, chunk_content: str, region: str = "") -> list[dict]:
        ...
        if region:
            prompt += f"\n\n本次导入已指定章节为“{region}”。所有输出条目的 domain 必须严格为 [\"{region}\"]。"
